...
/** Listener estable para MapLibre: lee siempre el handler más reciente vía ref. */
function useStableMapListener(handler: () => void) {
...
function useStableMapLayerHandler<E extends MapLibreGL.MapLayerMouseEvent>(
...
function MapRoute({
  ...
  useEffect(() => {
    ...
    return () => {
      map.off("click", layerId, onRouteClick);
      map.off("mouseenter", layerId, onRouteMouseEnter);
      map.off("mouseleave", layerId, onRouteMouseLeave);
    };
...
function MapArc<T extends MapArcDatum = MapArcDatum>({
  ...
  const onArcMouseMove = useStableMapLayerHandler(
    (e: MapLibreGL.MapLayerMouseEvent) => {
      ...
      if (featureId == null || featureId === arcHoverIdRef.current) return;
  ...
  const onArcMouseLeave = useStableMapListener(() => {
    if (!map) return;
    setArcHover(null);
  ...
  const onArcClick = useStableMapLayerHandler(
    (e: MapLibreGL.MapLayerMouseEvent) => {
  ...
  useEffect(() => {
    ...
    map.on("mousemove", hitLayerId, onArcMouseMove);
    map.on("mouseleave", hitLayerId, onArcMouseLeave);
    map.on("click", hitLayerId, onArcClick);

    return () => {
      map.off("mousemove", hitLayerId, onArcMouseMove);
      map.off("mouseleave", hitLayerId, onArcMouseLeave);
      map.off("click", hitLayerId, onArcClick);
      setArcHover(null);
      map.getCanvas().style.cursor = "";
    };
...
>({
  ...
  const onUnclusteredPointClick = useStableMapLayerHandler(
    (
      e: MapLibreGL.MapMouseEvent & {
        features?: MapLibreGL.MapGeoJSONFeature[];
      },
  ...
  const onClusterMouseEnter = useStableMapListener(() => {
    map?.getCanvas().style.setProperty("cursor", "pointer");
  });
  const onClusterMouseLeave = useStableMapListener(() => {
    map?.getCanvas().style.removeProperty("cursor");
  });
  const onPointMouseEnter = useStableMapListener(() => {
    if (onPointClickRef.current) {
      map?.getCanvas().style.setProperty("cursor", "pointer");
    }
  });
  const onPointMouseLeave = useStableMapListener(() => {
    map?.getCanvas().style.removeProperty("cursor");
  });
  ...
  useEffect(() => {
    ...
    map.on("click", unclusteredLayerId, onUnclusteredPointClick);
    map.on("mouseenter", clusterLayerId, onClusterMouseEnter);
    map.on("mouseleave", clusterLayerId, onClusterMouseLeave);
    map.on("mouseenter", unclusteredLayerId, onPointMouseEnter);
    map.on("mouseleave", unclusteredLayerId, onPointMouseLeave);

    return () => {
      map.off("click", clusterLayerId, onClusterLayerClick);
      map.off("click", unclusteredLayerId, onUnclusteredPointClick);
      map.off("mouseenter", clusterLayerId, onClusterMouseEnter);
      map.off("mouseleave", clusterLayerId, onClusterMouseLeave);
      map.off("mouseenter", unclusteredLayerId, onPointMouseEnter);
      map.off("mouseleave", unclusteredLayerId, onPointMouseLeave);
    };
